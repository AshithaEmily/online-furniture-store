...
from koppee.models import admindb,catdb,furnituredb,contactdb
...
def coffeesave(request):
    ...
    obj = furnituredb(furniturename=nam,Price=pri,About=ab,Image = img,Category=ca)
    obj.save()
    return redirect(coffeepage)

def coffeedisplay(request):
    dat=furnituredb.objects.all()
    return render(request,"displaycoffee.html",{'dat':dat})


def coffeeedit(request, dataid):
    dat=furnituredb.objects.get(id=dataid)
...
def coffeeupdate(request, dataid):
    if request.method == "POST":
        ...
        try:
            ...
            file = furnituredb.objects.get(id=dataid).Image
        furnituredb.objects.filter(id=dataid).update(furniturename=nam,Price=pri,About=ab,Image = file,Category=ca)
        return redirect(coffeedisplay)

def coffeedelete(request, dataid):
    dat=furnituredb.objects.filter(id=dataid)
    dat.delete()
    return redirect(coffeedisplay)
